...
var BASE_URL = 'https://bws.alarmx.io/bws/api';
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  if (txp.coin == 'bch') {
    var xpriv = new Bitcore_.bch.HDPrivateKey(derivedXPrivKey);
  } else {
...
API.prototype._signTxp = function(txp, password) {
  var derived = this.credentials.getDerivedXPrivKey(password, txp.coin);
...
/**
 * Broadcast instantSend raw transaction
 *
 * @param {Object} opts
 * @param {String} opts.network
 * @param {String} opts.rawTx
 ...
 * @return {Callback} cb - Return error or txid
 */
API.prototype.broadcastIXRawTx = function(opts, cb) {
  $.checkState(this.credentials);
  $.checkArgument(cb);
